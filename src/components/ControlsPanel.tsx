...
const CARD_STYLES: { id: IDCardStyleId; label: string; color: string }[] = [
  { id: 'studio-emerald', label: '2:47 PM Emerald', color: '#ffe500' },
  { id: 'terminal-dark', label: 'Terminal Dark', color: '#00f0ff' },
  { id: 'neon-ocean', label: 'Neon Ocean', color: '#38bdf8' },
  { id: 'sunset-blaze', label: 'Sunset Blaze', color: '#ff9100' },
  { id: 'holohack-gold', label: 'Holohack Gold', color: '#ffd700' },
];

const ACCENT_COLORS = [
  { name: 'Golden Yellow', hex: '#ffe500' },
...
export const ControlsPanel: React.FC<ControlsPanelProps> = ({
  ...
  const handleTitleGenerate = () => {
    ...
    setCardData((prev) => ({ ...prev, builderTitle: randomTitle }));
  };
    ...
    <div className={`glass-panel ${isSquare ? 'square-corners' : 'rounded-corners'}`} style={{ padding: '1.2rem', display: 'flex', flexDirection: 'column', gap: '1.2rem' }}>
      ...
      <div>
        <div className="input-label" style={{ marginBottom: '0.6rem' }}>
          <Layers size={16} /> STEP 2: SELECT {mode === 'pfp' ? 'FRAME OVERLAY' : 'CARD STYLE'}
        </div>
            ...
            : CARD_STYLES.map((style) => (
                <button
                  ...
                  style={{
                    ...
                    background: cardStyle === style.id ? 'var(--accent-yellow)' : 'transparent',
                    color: cardStyle === style.id ? '#043e24' : 'var(--text-main)',
                    fontWeight: cardStyle === style.id ? 900 : 700,
                    display: 'flex',
                    flexDirection: 'column',
                    gap: '0.2rem'
                  }}
